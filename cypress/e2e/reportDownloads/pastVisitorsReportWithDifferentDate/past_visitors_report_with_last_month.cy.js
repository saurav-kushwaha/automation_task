...
describe('Portal Downloads : quick reports  ', () => {
    ...
    it('should show confirmation message of quick report  after clicking on download for button of Past Visitors Report with last month',()=>{
        cy.wait(5000)
        quickReport.getClickOnDownloadCenter();
        quickReport.getClickOnQuickReport();
        quickReport.verifyTitleText().eq(7).should("contain.text", "Past Visitors Report")
        quickReport.getClickOnDatePicker().eq(3).click();
        quickReport.getClickOnLastMonth();
        quickReport.getClickOnDownloadForBtn().eq(3).click();
