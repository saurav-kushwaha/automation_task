...
describe(' CommandPalette Functionality',()=>{
   ...
   it('Verify  after clicking on Manage your bookings should redirect to my booking page',()=>{
        commandPalette.getCommandPalette().click()
        commandPalette.getSuggestionOption().each(($e1, index, $list) => {
            if($e1.text()==="Manage your bookings.")
            {
            cy.wrap($e1).click()
            }
        })
        cy.url().should('eq','https://ndl.veris.in/meeting-room/my-bookings')
